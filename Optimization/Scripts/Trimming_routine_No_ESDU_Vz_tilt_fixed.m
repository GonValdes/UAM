function [sol_final,Preq,P_req_forw,P_req_back] = Trimming_routine_No_ESDU_Vz_tilt_fixed(TAS,Vz,tilt,Altitude,pars,pars_BackProp,dir,filename,AerofoilDataset,neural)
% Function to Trim the aircraft at a certain flight condition with fixed tilt
%angle. Based on a brute force algorithm.
...
%   tilt: propeller tilt angle in deg
...
% 1 Thrust ForwProp (N) 2 Pitch angle body(rad) 3 Tilt(rad) 4 Elevator(deg)
% 5 Thrust BackProp(N) before applying multipliers
...
multipliers= [1/100,1/10,1,1,1/100];
% multipliers= ones(5,1)
T_range=[T_0-200:750:5000]*multipliers(1);
pitch_range = [0 5 10 15 20 30 40 50]*pi/180*multipliers(2);
tilt = tilt*multipliers(3)*pi/180;
% tilt_range=[0:10:90]*pi/180/10;
eta_range=[0 15]*multipliers(4);
T_backprop_range = [0:1500:14000]*multipliers(5);


%Multipliers for better convergence:
%Forw prop, pitch body, tilt, elevator, back prop

% Define options for trimming algorithm
fun = @(x)trim_funct_estimation_BackProp_Vz_tiltfixed(x,tilt,multipliers,gamma,initialise,geo,qbar,q_prop,prop_cg_loc,prop_cg_locback,CD,y_mac);
    ...
    for j=1:length(pitch_range)
        ...
        for l =1:length(eta_range)
            for m=1:length(T_backprop_range)
                ...
                x0 = [T_range(i),pitch_range(j),eta_range(l),T_backprop_range(m)];
                
%                 x0 = [3000 9*pi/180+gamma 1.5 1000];
%                 tilt = 0*pi/180;
%                 trim_funct_estimation_BackProp_Vz_tiltfixed(x0,tilt,multipliers,gamma,initialise,geo,qbar,q_prop,prop_cg_loc,prop_cg_locback,CD,y_mac)
                
                %Running the trimming algorithm
                [sol] = fsolve(fun,x0,opts);
                %Obtain the forces value and the solution
                F = trim_funct_estimation_BackProp_Vz_tiltfixed(sol,tilt,multipliers,gamma,initialise,geo,qbar,q_prop,prop_cg_loc,prop_cg_locback,CD,y_mac);
                %reapply multipliers
                sol(3)=sol(3)/multipliers(4);%deg
                
                sol(2)=sol(2)/multipliers(2);%rad
                
                %Condition for the validity of the solution
                if (abs(F(1))>100) || (abs(F(2))>100) || (abs(F(3))>100)|| sol(1) < 0 || (sol(2)-gamma) > 10.5*pi/180 || abs(sol(3)) > 30|| sol(4) < 0
                    continue % skip if conditions are not met
                else
                    sol(1)=sol(1)/multipliers(1);
                    sol(4)=sol(4)/multipliers(5);
